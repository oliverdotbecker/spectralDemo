...
\vspace{.6cm}
\par\noindent\rule{\textwidth}{0.4pt}
In partial fulfillment of the requirements of the \textbf{University of Applied Sciences Hochschule
Darmstadt (h\_da)} for the degree \textbf{Master of Science in Electrical Engineering} carried out in
collaboration with \textbf{Industrial Enterprise}\\
\\
...
\par\vspace{2cm}\noindent
Darmstadt, 31.10.2019 \hspace*{0pt}\hfill Oliver Becker
\clearpage
...
\\
In dieser Arbeit wird betrachtet, wie man anhand von spektralen Messdaten der LED-Lichtquellen von
Scheinwerfern einen automatisierten chromatischen Abgleich zwischen einem Referenzgerät und einem 
oder mehreren anzugleichenden Geräten realisieren kann.\\
...
\begin{wrapfigure}{r}{0.5\textwidth}
    \vspace{-25pt}
    \begin{center}
        \includegraphics[width=0.48\textwidth]{images/ma_wbb_luftbild_2.png}
    \end{center}
    \vspace{-20pt}
    \captionsetup{justification=raggedright,singlelinecheck=false}
    \caption[Firmensitz Waldbüttelbrunn]{Firmensitz Waldbüttelbrunn \cite{googleEarth}}
    \vspace{-5pt}
\end{wrapfigure}
...
einen Leitung mit invertiertem und auf der anderen mit nicht invertiertem Pegel. Da sich externe 
...
Verteilungen. Oder anders ausgedrückt: eine Farbvalenz kann durch viele Farbreizfunktionen $\phi_\lambda$ ausgedrückt werden."'\cite{lichtquellen}
\clearpage
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Typische Spektren der verschiedenen Leuchtmittel]{Typische Spektren der verschiedenen Leuchtmittel \cite{showlicht}}\label{fig:lightEmiiterSpects}
\end{figure}
\noindent
...
Das SkyPanel S60-C von der Firma Arri, ein Tochterunternehmen der Firma Osram, wird aufgrund ihrer Farbtreue und der Möglichkeit, dass
es mit \ac{cie} 1931 xy Werten per DMX angesteuert werden kann und diese \ac{cie} Werte von der Lampe exakt wiedergegeben werden, oft
als Grundlicht in Film und Fernsehen eingesetzt. Aufgrund dieser Fakten wurde diese Lampe als Referenz für diese Arbeit ausgewählt. %//Lesen
\begin{figure}[H]
    \begin{center}
...
Serie erfasst werden und im XLS Format für die weitere Auswertung auf einer SD Karte gesichert werden.
\clearpage
...
Das Sensorboard wurde mit einem "'FTDI Friend"', einem weiteren Produkt von Adafruit, mit dem Endgerät, einem Windows Computer,
verbunden. Dies ist ein USB-Seriell-Transceiver. Dieser Chip von der Firma \ac{ftdi} wandelt \ac{usb} zu RS232 Analogsignalen um. Die
Verbindung mit dem \ac{ic} wird über \ac{uart} via RS232 hergestellt, mit einer \ac{baud} Rate von 115200.\\
...
Die AT Schnittstelle stellt verschiedenste Informationen zur Verfügung. Zunächst können mit dem Befehl "'ATDATA"' die Sensorwerte als
...
angeschaltet werden, um nicht selbst emittierende Objekte zu vermessen.\\
...
\subsection{AS7262}
Das Sensorspektrum des AS7262 ähnelt sehr der Zapfenverteilung im menschlichen Auge. Dieser \ac{ic} hat insgesamt sechs einzelne
...
wurde von AMS separat bestellt. Anschießend wurde der Chip auf einem zweiten AS7262 Evaluierungsboard durch einen AS7261 ausgetauscht.\\
...
pro Lampe ein Ordner mit Unterordnern pro Emitter. Die XLS Dateien der Messungen wurden in diesen Unterordnern gesammelt. Es wird angenommen, dass die
...
Formeln (\ref{Equ:8}) bis (\ref{Equ:10}) auf Seite \pageref{Equ:10} möglich. Mit diesen Werten konnte nun der Punkt nach jeder Messung, also immer
...
um virtuell in der Applikation die Emitter additiv zu mischen. Dies wird erreicht, indem die gemessenen XYZ Koordinaten der zu mischenden Emitter aufaddiert werden.
...
Sobald der Benutzer einen Schieberegler betätigte, wurde die gemessene oder interpolierte XYZ Koordinate der Emitter aufaddiert. Diese kombinierte
XYZ Koordinate wurde in eine \ac{cie} xy Koordinate umgerechnet und zusätzlich zu der aktuellen Messung in der CIE 1931 Normtafel angezeigt.
\begin{figure}[H]
    \centering
...
übereinstimmt. Bei Messungen mit den anderen Lichtquellen ist diese Abweichung nicht aufgetreten. Die beiden Kreuze waren in jeder beliebigen
...
emittieren.\\
So wird bei einer 100 Prozent Ansteuerung des grünen Emitters eine Helligkeit von circa 4500 Lux von dem einen Emitter abgegeben. Bei der
...
dreieckiges Polygon als Gamut auf. Ein vierter weißer Emitter liegt in der Regel innerhalb dieses Polygons. Dies erweitert den Gamut nicht. Läge ein weiterer Emitter
außerhalb des bereits aufgespannten Polygons, wird dieses entsprechend erweitert.\\
Um den Gamut einer Lampe in ein CIE Diagramm einzuzeichnen, müssen die xy Koordinaten gegen den Uhrzeigersinn sortiert werden. Dies wird bewerkstelligt,
...
Der Algorithmus spannt die Gerade zwischen zwei Vertices (P1 und P2) des Gamuts A und zwischen zwei Vertices (CP1 und CP2) des Gamuts B. Anschließend wird geprüft prüft, auf
welcher Seite der Geraden des Gamuts A sich die Vertices des Gamuts B befinden.\\
Wenn beide Vertices sich auf der, gegen den Uhrzeigersinn gesehen, linken Seite, also der Innenseite (Gamutflächenseite) des Gamuts befinden, wird der
zweite Vertex (CP2) der Gamutvertexliste hinzugefügt.\\
Liegt der erste der beiden Vertices außerhalb der Gamutflächenseite, wird der auf der Gamutfläche liegende Vertex (OP2) und der Schnittpunkt des Polygons und der
Gamutflächenseite  (OP1) der Gamutvertexliste hinzugefügt (siehe Bild \ref{fig:sutherland_hodgman}).\\
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{.49\textwidth}
...
Dies wird für alle Kombinationen der Geraden des Gamut A und B durchgeführt.\\
...
zusammengeführt, bis alle Lampen der Selektion durchlaufen sind. Der hier entstehende kombinierte Gamut wird nun in dem CIE 1931 Diagramm weiß eingezeichnet.
Zusätzlich wird der Gamut jeder einzelnen Lampe in schwarz eingezeichnet (siehe Bild \ref{fig:combinedGamut}).
\begin{figure}[H]
    \begin{center}
...
in der Applikation eine \ac{dmx} Ausgabe via Art-Net implementiert. Weiter wurde ermöglicht, mehrere Lampen zu verwalten und zu steuern, um später mehrere
zu vermessende Lampen ansteuern und abgleichen zu können.
...
die Lampe reagiert. Hier wird je nach Anwendungsfall ein DMX Modus gewählt. Das SkyPanel bietet unter anderem in einem DMX Modus eine Ansteuerung von CIE 1931 x- und
y-Parametern auf je einem \ac{dmx} Kanal. Hierfür werden die auf diesen Kanälen empfangenen \ac{dmx} Werte in der Lampe in Helligkeitswerte für die einzelnen Emitter
übersetzt.\\
...
ausgegeben werden.\\
Die Spalte "'M"' steht für "'Measure"'. Mit dem Anklicken der Zelle wird später das automatisierte Vermessen des Fixtures angestoßen. Mit Klicken auf die
...
Zur Ansteuerung der Lampen mit \ac{dmx} wurde eine einfache Art-Net Implementierung aus dem NPM-Modul "'artnet"' von dem Autor mit dem Alias "'hobbyquaker"'
genutzt \cite{artnetPackage}. Zur Umsetzung von Art-Net zu DMX wurde ein grandMA3 "'4Port Node"' genutzt. Dieser wurde als Protokollwandler eingesetzt.\\
\begin{figure}[H]
    \begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%% Automatisiertes Abgleichen zweier LED-Lichtquellen %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
und dem gesuchten Mixpunkt (D) berechnet. Setzt man das Kreuzprodukt gleich Null, liegen die Vektoren im rechten Winkel zueinander.
...
\end{align}
Mit der Limitierung $0 \geq t \leq 1$ kann der errechnete Punkt nur auf der Strecke AB liegen, falls D, wie in Bild \ref{fig:refCombined} zu sehen, ebenfalls außerhalb des
kombinierten Gamuts liegt.\\
...
\end{lstlisting}
Diese Prozedur wird für alle Seiten des kombinierten Gamuts ausgeführt. Falls mehrere Punkte gefunden werden, wird der Punkt mit der kürzesten Vektordistanz zum
...
Um von dem Referenzpunkt zum Mixpunkt zu gelangen, muss grafisch betrachtet etwas von dem weißen Emitter hinzugegeben werden. Nicht direkt ersichtlich
...
einem Rechtsklick als Referenz definiert werden. Somit wird die vorberechnete Koordinate dieses Fixtures zum "'Referenzpunkt"'. Die gewählte Reihe wird mit roter
...
konnten.\\
Somit wird eine Farbortverschiebung innerhalb von zwei MacAdam Ellipsen als nicht wahrnehmbar angenommen. \cite{macAdamEllipses}\\
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=\textwidth]{images/CIExy1931MacAdam.png}
    \end{center}
    \vspace{-.4cm}
...
über ein Spektrum von 380 Nanometern jedoch zu gering, da zu viele Varianten aus den Wertekombinationen der Messwerte entstehen
...
finden.\\
Auch bei der Messung des gelben Mischtons ist eine erhebliche Diskrepanz zwischen dem vorberechneten Mixpunkt und dem
...
Sensoren, wäre ein deutlich genauerer Abgleich verschiedener LED-Lichtquellen möglich. Eine Kalibrierung ist immer
...
Eine Alternative könnte der Sensor AS7265x \cite{datasheet65} der Firma AMS sein. Dieser Sensor besteht aus drei
...
\\
Eine weitere Verbesserung des Ergebnisses könnte durch einen, zusätzlich von den Herstellern zur Verfügung gestellten, 
Roh-Ansteuerungsmodus für Lampen mit nichtlinearen internen Algorithmen zur Optimierung der Farbmischung erreicht werden. 
Diese Algorithmen werden in der Regel von Herstellern implementiert,
damit die verschiedenen Lampen eines Herstellers sich chromatisch möglichst gleich verhalten.
Eine Möglichkeit zur Umgehung dieser Algorithmen, um Lampen herstellerübergreifend miteinader abgleichen zu können, wäre
wünschenswert.\\
\clearpage
\noindent
Zusammengefasst ist der in dieser Arbeit entwickelte Prozess ein großer Schritt zum Abgleichen von LED-Lichtquellen.
In weiteren Schritten muss unter anderem ein geeigneter Spektralsensor zur Erfassung der Messdaten in die Applikation eingebunden
werden. Die Applikation wurde so programmiert, dass diese Erweiterung jederzeit vorgenommen werden kann, ohne die
weiteren Prozesse, wie den Abgleich, anpassen zu müssen.\\
Ein Einsatz in der Praxis ist zur Zeit noch nicht abzusehen.
\clearpage
...
\begin{thebibliography}{xxxxxxxxxxxxxxxxxxx}
    \bibitem[Google Earth, 2019]{googleEarth}"'Google Earth"', \url{https://earth.google.com/web/@49.79593509,9.84039498,314.18261337a,120.70738457d,35y,160.49228636h,59.99892202t,360r}. Visited on: 30.10.2019
    ...
    \bibitem[DMX-512-A Paper, 2019]{dmxstandard}"'DMX-512A Standard', \url{https://tsp.esta.org/tsp/documents/docs/E1-27-1_2006R2016.pdf}. Visited on: 13.10.2019
    ...
    \bibitem[Lichtquellen, 2015]{lichtquellen} Dohlus, Rainer: "'Lichtquellen"'. De Gruyter, Berlin, 2015. Page 62 + 63.
    \bibitem[Wisotop Homepage, 2019]{wisotopTriChro} "'Trichromatisches Sehen"', \url{https://wisotop.de/wellenlaengen-des-lichts-farben.php}. Visited on: 03.10.2019
    ...
    \bibitem[Wisotop Homepage, 2019]{wisotopRGB} "'Das RGB-Farbmodell"', \url{https://wisotop.de/Farbmodelle-RGB-CMYK-HSS-HSL.php}. Visited on: 27.09.2019
    \bibitem[RGB-Farbraum - Wikipedia, 2019]{wikipediaRGB} "'RGB-Farbraum"', \url{https://de.wikipedia.org/wiki/RGB-Farbraum}. Visited on: 29.09.2019
    \bibitem[Hermann Graßmann - Wikipedia, 2019]{wikipediaGrassmann} "'Hermann Graßmann"', \url{https://de.wikipedia.org/wiki/Hermann\_Gra%C3%9Fmann}. Visited on: 29.09.2019
    \bibitem[Graßmannsche Gesetze - Wikipedia, 2019]{wikipediaGrassmannGestze} "'Graßmannsche Gesetze"', \url{https://de.wikipedia.org/wiki/Gra%C3%9Fmannsche_Gesetze}. Visited on: 29.09.2019
    \bibitem[CIExy1931 - Wikipedia, 2019]{wikipedia1931Image} "'CIExy1931.svg"', \url{https://de.wikipedia.org/wiki/Datei:CIExy1931.svg}. Visited on: 30.09.2019
    \bibitem[CIE - Wikipedia, 2019]{wikipediaCIE} "'CIE-Normvalenzsystem"', \url{https://de.wikipedia.org/wiki/CIE-Normvalenzsystem}. Visited on: 30.09.2019
    \bibitem[CIELUV - Wikipedia, 2019]{wikipedia1976} "'CIELUV-Farbraumsystem"', \url{https://de.wikipedia.org/wiki/CIELUV-Farbraumsystem}. Visited on: 30.09.2019
    \bibitem[CIE Luv - Wisotop, 2019]{wisotopLuv} "'Der LUV-Farbraum der CIE"', \url{https://wisotop.de/CIE-luv.php}. Visited on: 01.10.2019
    ...
    \bibitem[Showlicht von Scribble zum fertigen Desgin, 2014]{showlicht} Greil, Roland; Lorenz, Marc: "'Showlicht von Scribble zum fertigen Desgin"'. LEIKRO Media GmbH, Gröbenzell, 2014. Page 35 + 37.
    \bibitem[Arri Homepage, 2019]{arriL7} "'LC-7"', \url{https://www.arri.com/en/lighting/led/l-series/l7-c#F0.0000L7C}. Visited on: 29.10.2019
    \bibitem[Ape Labs Homepage, 2019]{lightCan} "'LightCan"', \url{https://apelabs.com/products/leuchtdose}. Visited on: 02.09.2019
    \bibitem[Robe Homepage, 2019]{robe} "'LEDWash 300+"', \url{https://www.robe.cz/ledwash-300plus/}. Visited on: 02.09.2019
    \bibitem[Arri Homepage, 2019]{arri} "'S60-C"', \url{https://www.arri.com/en/lighting/led/skypanel/s60-c}. Visited on: 02.09.2019
    \bibitem[UPRtek Homepage, 2019]{uprtek} "'LED Test \& Measurement-Handspektrometer"', \url{https://www.uprtek.com/de/product/LED-Test-Measurement-Handspektrometer/MK350N-Basic-Handheld-Spectrometer.html}. Visited on: 09.09.2019
    \bibitem[AMS Homepage, 2019]{ams} "'About ams"', \url{https://ams.com/about-ams}. Visited on: 04.10.2019
    ...
    \bibitem[Lindbloom Homepage, 2019]{lindbloom} "'Bruce Lindbloom"', \url{http://brucelindbloom.com/index.html}. Visited on: 09.09.2019
    \bibitem[Wisotop Homepage, 2019]{wisotop} "'Spectral Tristimulus Values Defining CIE 1964"', \url{https://wisotop.de/Anhang-Tristimulus-Werte.php}. Visited on: 09.09.2019
    \bibitem[LightCan Manual, 2019]{lightCanDMX} "'LightCan Bedienungsanleitung"', \url{http://download.apelabs.com/manuals/Ape_Labs_-_LightCan_-_Manual_-_web.pdf} Page 33. Visited on: 16.09.2019
    \bibitem[SkyPanel Manual, 2019]{skypanelDMX} "'SkyPanel DMX Protocol Specification"', \url{https://www.arri.com/resource/blob/65958/560471fec7f8f2e186ef8ca9e6cdd489/arri-skypanel-dmx-protocol-specification-v4-4-en-okt2018-data.pdf} Page 12. Visited on: 16.09.2019
    \bibitem[artnet NPMJS, 2019]{artnetPackage} "'artnet"', \url{https://www.npmjs.com/package/artnet}. Visited on: 16.09.2019
    \bibitem[Sutherland–Hodgman algorithm Wikipedia, 2019]{sutherlandHodgman} "'Sutherland–Hodgman algorithm"', \url{https://en.wikipedia.org/wiki/Sutherland%E2%80%93Hodgman_algorithm}. Visited on: 22.09.2019
    \bibitem[David MacAdam Wikipedia, 2019]{macAdam} "'David MacAdam"', \url{https://en.wikipedia.org/wiki/David_MacAdam}. Visited on: 18.10.2019
    \bibitem[Optibin, 2019]{macAdamEllipses} "'Optibin - Achieves color consistency with industry-leading LED optimization"'\url{https://www.colorkinetics.com/b-dam/color-kinetics/learn-more/intellihue/PCK-Technology-Overview-Optibin.pdf}. Page 7. Visited on: 28.10.2019
    \bibitem[LED ColorCalculator, 2019]{osram} "'LED ColorCalculator"', \url{https://www.osram.us/cb/tools-and-resources/applications/led-colorcalculator/index.jsp}. Visited on: 18.10.2019
    \bibitem[AS7265x Datenblatt, 2019]{datasheet65}"'AS7265x Smart 18-Channel VIS to NIR Spectral ID 3-Sensor Chipset with Electronic Shutter"', \url{https://ams.com/documents/20143/36005/AS7265x_DS000612_1-00.pdf/08051c8a-a7f6-6231-7993-2d3fe0bf38b8}. Visited on: 26.10.2019
\end{thebibliography}
\endgroup
\clearpage
