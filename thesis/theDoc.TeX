...
auf Kleinveranstaltungen. In den frühen Jahren der großen Konzerte wurden hauptsächlich Scheinwerfer mit Glühfaden Leuchtmitteln beleuchtet.
...
gasgefüllten Röhre zum Einsatz. Mit dem dem Anlegen einer Spannung, an den beiden Elektroden baut ein elektrisches Feld auf, welches zu
...
\end{figure}
\noindent
In den 2000er Jahren hielt dann die LED Einzug in die Welt der Veranstaltungstechnik. Dies begann einen Wandel, was besonders die Lebensdauer
der Leuchtmittel betrifft, sowie der Lichtausbeute im Vergleich zum Stromverbrauch. Mit der Fortschreitenden Entwicklung der \ac{led} und der
...
hat nur eine Lebensdauer von etwa 300 Betriebstunden. Eine Gasentladungslampe hält einer Betriebsbelastung von etwa 300 bis 2000 Stunde aus.
...
Lichtinstallationen umgesetzt werden. Da die Verfügbare Leistung an Veranstaltungsorten begrenzt ist konnte mit Konventionellen \ac{par}
Scheinwerfern nur, im Vergleich zu Heute, geringe Zahlen eingesetzt werden, da schon mit 100 \ac{par}s mit 1000W Anschlussleistung bereits
...
Vergangenheit dennoch bei Großveranstaltungen solche Stückzahlen in Bühnen verbaut und dies nicht als einziger Scheinwerfertyp.
Denn hier hat die LED einen weiteren Vorteil zu damals.\\
Früher wurden um eine Bühne in verschiedenen Farben auszuleuchten mehrere \ac{par}s benötigt. Um nur Rot, Grün, Blau und Weiß leuchenten zu
...
Somit konnte ein deutlich homogeneres Abstrahlverhalten erreichtwerden.
\clearpage

\section{Menschliches Farbsehen}
Das menschliche Auge besitzt zwei Arten von "'Sensoren"', oder auch genannt Rezeptoren. Stäbchen zur Helligkeitswahrnehmung bei Nacht.
...
\end{figure}
Die Mengenverteilung der verschiedenen Zapfen Arten liegt circa bei S:M:L 1:16:32.
\clearpage
...
Die Schnittstelle stellt noch einige weiter Inforamtionen zur Verfügung wie die Chip Software-Version, einen Sensor Gain, Sensor Bank Modus
...
Bereich. So sind auch in Bild \ref{fig:eye_spectrum} zwei der drei Zapfen Arten im Rot bis Grünen Bereich. Die dritte Zapfen Art ist im blauen
Bereich.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.49\textwidth}
...
\end{figure}
\noindent
Hier wird aus der \ac{uart} (AT) Schnittstelle mit dem Befehl "'ATCDATA"' die Sensorwerte der einzelnen Dioden als 32-Bit floating point
...
\section{Verwendete Lichtquellen}
...
\end{wrapfigure}
\noindent
Diese Lampe ist ein sehr kompakt gebaute RGBW\ac{led} Lichtquelle auf Akku Basis. Diese wurde von der Firma Ape Labs aus Eisigen in Deutschald
entwickelt. Diese Lampe ist sowohl per Fernbedienung, als auch per W-\ac{dmx} steuerbar.
Die "'Kanne"' hat einen Abstrahlwinkel von 65 Grad. Zur gezielten Steuerung, ohne vorprogrammierte Effekte wird ein \ac{dmx} zu W-DMX Wander der
Firma benötigt (W-APP). Dieser ermöglicht dann die Ansterung der Lampe von der Applikation.
\subsection{Robe LEDWash 300}
...
\end{figure}
\noindent
Das Movinglight hat einen einstellbaren Abstrahlwinkel von 8 bis 63 Grad. Dieser wurde allerdings bei den Messungen auf 63 Gard fest eingestellt,
...
\subsection{ARRI SkyPanel}
Das Skypanel S60-C von der Firma Arri, ein Tochterunternehmen der Firma Osram, wird oft als Fluter, Grundlicht in Film und Fernsehen eingesetzt.
Es wird beworben, dass es mit xyY Kanälen per DMX angesteuert werden kann, diese CIE Werte von der Lampe exakt wiedergegeben werden. Somit
ist diese Lampe eine perfekte Referenz für diese Arbeit.
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=.48\textwidth]{images/skypanel_s60_c.png}
    \end{center}
    \caption{ARRI Skypannel S60-C \cite{arri}}
\end{figure}
\noindent
...
Bei einer Ansteuerung per DMX oder Art-Net kann am Gerät auch abergelesen werden, welche xyY Werte das Gerät aus der Ansteuerung ermittelt,
