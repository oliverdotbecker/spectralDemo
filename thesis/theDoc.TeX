...
\vspace{1cm}\noindent
\begin{center}
    \huge \color{red} Vorabversion 0.9\\
    \large Stand: \today
\end{center}
\vspace{1cm}\noindent
...
Der Begriff der Farbtemperatur ist durch den planckscher Strahler begründet. Es wurde beobachtet, dass bei dem starken Erhitzen von einem Metall
sich die Farbe des Metalls verändert. Ab einer Temperatur von etwa 923 Kelvin (K), also 650 Celsius, fängt das Metall an rot zu glühen. Bei
...
Halterung kann eines der Sensorboards eingeschoben werden. Mit einem definierten Abstand von zwölf Zentimetern kann in den rechen runden Halterungen
...
In der Applikation wurden die Emitter der Light Can aufgelistet. Diese sind Rot, Grün, Blau und Weiss. Pro Emitter werden acht Helligkeitsstufen
...
Um die gemessenen Werte in der Applikation zu erfassen, wurde eine Emitter Sektion in der grafischen Benutzeroberfläche vorgesehen.
...
\end{figure}
\noindent
Das Emitter Menü bot einige Einstellungsmöglichkeiten wie den Namen und die Farbe zur bessern Zuordnung für den Benutzer, sowie pro zu messende
Helligkeitsstufe eine Schaltfläche. Während einer Messung musste der Benutzer nun mit einer externen (DMX) Ansteuerung, Beispielsweise den roten
...
Mit den nun verfügbaren Messwerten einer Lichtquelle wurde zur Überprüfung der bisher eingesetzten Berechnugnen eine Möglichkeit programmiert,
um virtuell in der Applikation die Emitter additiv zu mischen. Durch Interpolation zwischen den erfassten Heliggkeitsstufen und additiver
Farbmsichung den Farbton einer aktiven Emitterkombination vorauszuberechnen.
\begin{figure}[H]
    \begin{center}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Zwischenstand der Benutzeroberfläche mit Messungen und Vorberechung einer Light Can}
\end{figure}
\noindent
...
angezeigt wird.\\
Hier ist zu beobachten, dass bei einem Mischfarbton in Bild \ref{fig:calcLightCanMix} die Messung der Lichquelle mit der Vorberechnung nicht
übereinstimmt. Bei Messungen mit den anderen Lichtquellen ist diese Abweichung nicht aufgetreten. die zwei Kreuze waren zumeist in jeder beliebeigen
Kombination nahezu deckungsgleich.\\
Diese Abweichung entsteht durch eine spezielle Funktion des Light Can Scheinwerfers. Die Light Can nutzt einen interen Algorithmus, um sowohl bei
maximaler Austeuerung eines einzelnen Emitters als auch bei der Aussteuerung von mehereren Emittern immer die gleiche Gesamthelligkeit zu
emmitieren.\\
...
Kombination mehrere Emitter wird die maximale zu erriechende Helligkeit der einzlenen Emitter reduziert.\\
\\
Dieser Test zeigt, dass für lineare und annähernd linare Farbmischsysteme eine einzel Emittervermessung ausreichend ist um den Gammut, den möglichen
Farbraum und die Farbmischung vorauszuberechnen. Für nicht linare Farbmischsysteme sind weitere Messungen der tatsächilchen Mischfarben nötig oder der
...
Zur automatisierung der Vermssung der Lichtquellen ist es nötig, dass die messende Applikation auch die Lichtquellen ansteuern kann. Somit wurde
in der Applikation eine \ac{dmx} Steuerung mit Art-Net implemementiert. Weiter wurde ermöglicht, mehere Lampen zu verwalten und zu steuern, um später meherere
...
die hier venrwendeten Lichtquellen, bis zu Bewegungssteurungen meherer Achsen, wie des Robe LEDWash 300.\\
Die Zuordung dieser Funktionen zu den DMX Steuerkanälen der Lampe ist somit je nach Funktionalitätsumfang von Lampe zu Lampe und auch bei einem gleichen
...
% Bild DMX Chart Light Can und SkyPanel
Das SkyPanel hingegen bietet {\color{red} xx} sogenannte DMX Modi. Je nach Wahl des DMX Modus wechselt die Belegung der \ac{dmx} Steuerkanäle, auf die
...
y Kanälen auf zwei \ac{dmx} Kanälen. Dies ermöglicht die Lampe durch internes Umsetzen der empfangenen \ac{dmx} Werte auf diesen Kanälen in Helligkeitswerte
für die einzelnen Emitter.\\
Eine abstrahierte, oder auch vereinheitliche Steuerung für equivalente Lampenfunktionen ist durch diese Unterschiede in der Belegung nur durch
...
\subsection{Patch}
Im sogenannten "'Patch"' oder auch "'Addressierung"' legt man fest welche Lampen man steuern möchte. Ein Patch Eintrag besteht aus den Inforamtionen,
des verwendeten Fixturetypen und der DMX Startaddresse.
