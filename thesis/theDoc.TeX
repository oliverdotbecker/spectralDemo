...
Vertex der Gamutvertex Liste hinzugefügt.\\
Liegt der erste der beiden Vertices außerhalb der Gamutflächenseite, wird der auf der Fläche liegende Vertex und der Schnittpunkt des Polygons und der
...
\end{figure}
\noindent
Liegt der zweite Vertex außerhalb der Gamutflächenseite wird der Schnittpunkt des Polygons und der Gamutflächenseite der Gamutvertex Liste hinzugefügt.
...
Gamut wird mit dem der nächsten Lampe zusammengeführt. Der daraus entstehende kombinierte Gamut wird nun mit dem der nächsten selektierten Lampe ebenfalls
...
Für eine automatisierte Vermessung der Lichtquellen ist es nötig, dass die messende Applikation auch die Lichtquellen ansteuern kann. Somit wurde
...
Lampentyp von Hersteller zu Hersteller unterschiedlich. So findet sich beispielsweise bei der LightCan kein Dimmer Steuerkanal für die Gesamtintensität
...
\end{figure}
\noindent
Das SkyPanel hingegen bietet 31 sogenannte DMX Modi, die LightCan 3 Modi. Je nach Wahl des DMX Modus wechselt die Belegung der \ac{dmx} Steuerkanäle, auf die
...
Zuletzt wird der Patch gesichert und beim nächsten Öffnen der Applikation auch wieder geladen.
...
\begin{addmargin}[2em]{0em}
    1. Wie kann ein Farbton außerhalb des kombinierten Gamut mit möglichst geringer Abweichung der Farbvalenz in den Bereich des kombinierten Gamut
    verschoben werden?\\
...
\end{addmargin}
Diese Punkte werden nun nacheinander erörtert. Die Ausgangssituation ist, dass ein Benutzer in der Applikation zwei vermessene Lampen selektiert. Die
...
dieses Polygons gefunden werden, sodass alle gewählten Lichtquellen in der Lage sind diese Farbvalenz zu erzeugen.\\
Zunächst wird geprüft, ob sich der Referenzpunkt innerhalb des kombinierten Gamuts befindet. Die Koordinaten des kombinierten Gamuts werden gegen den
...
der Referenzpunkt links von der Geraden zweier aufeinanderfolgender Koordinaten ist. Auch hier ist zu beachten, dass links von der Strecke aus Sicht der
Richtung der Geraden gegen den Uhrzeigersinn ist. Wenn diese Bedingung für alle Geraden erfüllt ist, befindet sich der Referenzpunkt innerhalb des kombinierten
Gamuts.\\
...
Falls der Referenzpunkt nicht außerhalb des kombinierten Gamuts liegt, wird der Referenzpunkt als Mixpunkt übernommen.\\
\\
...
werden, da je weiter ein Emitter im Farbraum von dem zu mischenden Punkt entfernt ist, desto geringer die Intensität des Emitters in der Mischung ist.
...
eingezeichnet.\\
Die Einzeichnung der vorberechnen Koordinate wurde erweitert, sodass diese für jedes selektiertes Fixture eingezeichnet wird. Jede Koordinate wird mit
...
Die errechneten und korrigierten Intensitätswerte werden im Fixture Sheet pro Emitter in Klammern hinter dem Nutzer eingestellten Wert angezeigt. Diese Werte
...
\end{figure}
\noindent
In Bild \ref{fig:measureScribble} wird der Messaufbau schematisch skizziert. Oben von links nach rechts sind die verwendeten Geräte eingezeichnet.
...
\end{table}
\noindent
Die Abstände wurden bei der Konzeption des Aufbaus nicht beachtet, da in der Praxis die Lampen für den Showeinsatz positioniert werden und nicht
...
zu sehen bereits bei 5\% Steuerwert eine höhere absolute Helligkeit hat als der rote Emitter bei 20\% Steuerwert. So ist schon bei minimalen Zumischen des weißen
...
Somit wird angenommen, dass eine Farbortverschiebung von bis zu zwei MacAdam Ellipsen als nicht wahrnehmbar angenommen.\\ %//TODO: Quelle von Daniel
\\
...
seiner Spektralenauflösung das Lichtspektrum nicht homogen aufnehmen kann. Somit entstehen auch bei der Farbortbestimmung große Abweichungen.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{.49\textwidth}
...
abzugleichenden Geräte ebenfalls fernab des Farborts des Referenzgeräts liegen. Jedoch liegen die Farborte der abzugleichenden
...
anhand der erfassten und weiter interpolierten Daten einer Lampe die gewünschte Mischfarbe zu produzieren. Hier
...
200 Millisekunden lassen sie nur eine begrenzte Messgeschwindigkeit zu. Weiter musste bei Messungen mit dem AS7262
...
maximalen Wert für den roten Emitter aus. Wenn nun der grüne Emitter mit 100\% hinzu genommen wird, wird der rote Emitter
...
Erfahrungen dieser Arbeit wären 5\% Schritte zu empfehlen, um Genauigkeit und Messzeit in ein ausgewogenes
...
Ein solcher geeigneter Sensor wäre mit einer entsprechender Kalibrierung der AS7261. Eine solche Kalibrierung ist
...
\\
Künftig wird diese oder eine ähnliche Implementierung in der grandMA3 Software der Firma MA Lighting zu Einsatz kommen.
